...
def stats_view(request):
    # Get user's expenses for the last 6 months
    todays_date = datetime.date.today()
    six_months_ago = todays_date - datetime.timedelta(days=30*6)
    ...
    category_data_list = [(category, amount) for category, amount in category_data.items()]
    
    # Calculate monthly total
    current_month = todays_date.replace(day=1)
    monthly_expenses = expenses.filter(date__gte=current_month)
    ...
    daily_average = sum(float(expense.amount) for expense in recent_expenses) / 30 if recent_expenses else 0
...
@login_required(login_url='/authentication/login')
def monthly_expense_data(request):
    """API endpoint to get monthly expense data for charts"""
    ...
    monthly_data = (
        Expense.objects
        .filter(owner=user, date__year=current_year)
    ...
    for item in monthly_data:
        ...
        monthly_expense_data[month_index] = float(item['total_expense'])
    
    return JsonResponse({'monthly_expense_data': monthly_expense_data})
