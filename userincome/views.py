...
def income_summary(request):
    ...
    income_sources = user.userincome_set.values('source').annotate(
        amount=Sum('amount'),
    ...
    recent_income = user.userincome_set.order_by('-date')[:5]
    
    # Calculate totals
    total_income = user.userincome_set.aggregate(Sum('amount'))['amount__sum'] or 0
    
    # Calculate average monthly income (last 6 months)
    six_months_ago = today - timedelta(days=180)
    last_six_months = user.userincome_set.filter(date__gte=six_months_ago)
    average_monthly = last_six_months.aggregate(Sum('amount'))['amount__sum'] or 0
    average_monthly = average_monthly / 6 if average_monthly else 0
...
def monthly_income_data(request):
    ...
    # Get the monthly income data for the current year, grouped by month, filtered by user
    monthly_data = (
        UserIncome.objects
        .filter(owner=user, date__year=current_year)  # Filter for the current year and user
    ...
    for item in monthly_data:
        ...
        monthly_income_data[month_index] = float(item['total_income'])
...
def generate_report(request):
    if request.method == "POST":
        ...
        context = {
            'income': incomes,  # Changed from 'incomes' to match template
            'expenses': expenses,
            'total_income': total_income,
